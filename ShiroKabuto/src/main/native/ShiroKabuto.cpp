...
const wchar_t *achOldlShiroAccount = L"ShiroKabuto";
const wchar_t *achNewShiroAccount = L"SoffidLocalAdmin";
const wchar_t *achShiroAccount = achNewShiroAccount;
...
void loop ()
{
	...
	if (result == NERR_UserNotFound) {
		achShiroAccount = achNewShiroAccount;
		result = NetUserGetInfo(NULL, // Local Host
				achShiroAccount, 2, // level
				&buffer);
	}
	...
	if (result == NERR_UserNotFound)
	{
		...
		if (notifyNewPassword(pass))
		{
			...
			ui2.usri2_full_name = (wchar_t*) L"Soffid local account";
...
void __stdcall shiroServiceStart (DWORD argc, LPTSTR *argv)
{
	...
	time_t now = 0;
	time (&last);
	while (WAIT_OBJECT_0 != WaitForSingleObject(hStopEvent, 300000)) // 5 minutes
	{
		updateServiceStatus(SERVICE_RUNNING);
		time (&now);
		if (now - last < 240) { // 4 minutes
			Sleep(240000);
		}
		time (&last);
