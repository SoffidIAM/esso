...
HRESULT __stdcall SSOCredential::QueryInterface (REFIID riid, void **ppObj)
{
...
ULONG __stdcall SSOCredential::AddRef ()
{
	return InterlockedIncrement(&m_nRefCount);
}

ULONG __stdcall SSOCredential::Release ()
{
	long nRefCount = 0;
...
HRESULT SSOCredential::Advise (ICredentialProviderCredentialEvents* pcpce)
{
...
HRESULT SSOCredential::UnAdvise ()
{
...
HRESULT SSOCredential::SetSelected (BOOL* pbAutoLogon)
{
	m_log.info("Credential::SetSelected");
	updateLabel();
	*pbAutoLogon = autoLogin;
	return S_OK ;
}
...
HRESULT SSOCredential::SetDeselected ()
{
...
//
HRESULT SSOCredential::GetFieldState (DWORD dwFieldID,
...
//
HRESULT SSOCredential::GetStringValue (DWORD dwFieldID, PWSTR* ppwz)
{
	HRESULT hr;
...
//
HRESULT SSOCredential::GetBitmapValue (DWORD dwFieldID, HBITMAP* phbmp)
{
	HRESULT hr;
...
//
HRESULT SSOCredential::GetSubmitButtonValue (DWORD dwFieldID,
		DWORD* pdwAdjacentTo)
{
	HRESULT hr;
...
//
HRESULT SSOCredential::SetStringValue (DWORD dwFieldID, PCWSTR pwz)
{
	HRESULT hr;
...
HRESULT SSOCredential::GetCheckboxValue (DWORD dwFieldID, BOOL* pbChecked,
		PWSTR* ppwzLabel)
{
...
HRESULT SSOCredential::GetComboBoxValueCount (DWORD dwFieldID, DWORD* pcItems,
		DWORD* pdwSelectedItem)
{
	return E_INVALIDARG ;
}

HRESULT SSOCredential::GetComboBoxValueAt (DWORD dwFieldID, DWORD dwItem,
		PWSTR* ppwzItem)
{
	return E_INVALIDARG ;
}

HRESULT SSOCredential::SetCheckboxValue (DWORD dwFieldID, BOOL bChecked)
{
	UNREFERENCED_PARAMETER(dwFieldID);
...
HRESULT SSOCredential::SetComboBoxSelectedValue (DWORD dwFieldId,
		DWORD dwSelectedItem)
{
	return E_INVALIDARG ;
}

HRESULT SSOCredential::CommandLinkClicked (DWORD dwFieldID)
{
	UNREFERENCED_PARAMETER(dwFieldID);
	return E_NOTIMPL ;
}
...
//
HRESULT SSOCredential::GetSerialization (
...
bool SSOCredential::ObtainCredentials (
		CREDENTIAL_PROVIDER_GET_SERIALIZATION_RESPONSE* pcpgsr,
		CREDENTIAL_PROVIDER_CREDENTIAL_SERIALIZATION* pcpcs,
		PWSTR* ppwzOptionalStatusText,
		CREDENTIAL_PROVIDER_STATUS_ICON* pcpsiOptionalStatusIcon)
{
	...
	if (!autoLogin) {
		this->szUser.assign(m_rgFieldStrings[SHI_USER]);
		this->szPassword.assign(m_rgFieldStrings[SHI_PASSWORD]);
	}
	return true;
}

HRESULT SSOCredential::GenerateLoginSerialization (
		CREDENTIAL_PROVIDER_GET_SERIALIZATION_RESPONSE* pcpgsr,
		CREDENTIAL_PROVIDER_CREDENTIAL_SERIALIZATION* pcpcs,
		PWSTR* ppwzOptionalStatusText,
		CREDENTIAL_PROVIDER_STATUS_ICON* pcpsiOptionalStatusIcon)
	...
	if (SUCCEEDED(hr))
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			if (SUCCEEDED(hr))
			{
				...
				pcpcs->clsidCredentialProvider = CLSID_SSO;
...
// being disabled.
HRESULT SSOCredential::ReportResult (NTSTATUS ntsStatus, NTSTATUS ntsSubstatus,
		PWSTR* ppwzOptionalStatusText,
		CREDENTIAL_PROVIDER_STATUS_ICON* pcpsiOptionalStatusIcon)
{
...
void SSOCredential::displayMessage ()
{
...
void SSOCredential::updateLabel() {
	if (autoLogin) {
		m_log.info("Setting title %ls", szUser.c_str());
		Utils::duplicateString(m_rgFieldStrings[SHI_TITLE], szUser.c_str());
		m_cpfs[SHI_TITLE] = CPFS_DISPLAY_IN_BOTH;
		if (m_pCredProvCredentialEvents != NULL) {
			m_pCredProvCredentialEvents->SetFieldString(this, SHI_USER, szUser.c_str());
			m_pCredProvCredentialEvents->SetFieldString(this, SHI_PASSWORD, szPassword.c_str());
			m_pCredProvCredentialEvents->SetFieldString(this, SHI_TITLE, szUser.c_str());
			m_pCredProvCredentialEvents->SetFieldState(this, SHI_PASSWORD, CPFS_HIDDEN);
			m_pCredProvCredentialEvents->SetFieldState(this, SHI_USER, CPFS_HIDDEN);
			m_pCredProvCredentialEvents->SetFieldState(this, SHI_TITLE, CPFS_DISPLAY_IN_BOTH);
		}
	}
}

void SSOCredential::hideMessage ()
{
