...
void Pkcs11Configuration::setNotificationHandler(SayakaProvider *provider) {
	m_provider = provider;
}
	...
	if (SCardEstablishContext(SCARD_SCOPE_SYSTEM, NULL, NULL, &cardCtx) == SCARD_S_SUCCESS)
	{
		while (true) {
			...
			do {
				...
				{
					m_log.info ("Exit token inspector");
					return;
				}
