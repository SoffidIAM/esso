 ...
 \
	...
	if (rv == CKR_OK) { \
		if (!(attr.pValue = calloc(1, attr.ulValueLen + 1))) \
			fprintf(stderr, "out of memory in get" #ATTR ": \n"); \
		rv = m_p11->C_GetAttributeValue(sess, obj, &attr, 1); \
		if (pulCount) \
			...
			if ((value = getVALUE(sess, obj, &sizeValue)) != NULL
				...
				if (pCertContext != NULL) {
					...
					if (strncmp (cert->getIssuer(), "AC DNI", 6) == 0 &&
						...
						printf ("Ignoring DNIE signature cert\n");
						delete cert;
						return ;
					}
					bool isCA = false;
					// Si el certificat és vàlid
...
std::vector<CertificateHandler*>& Pkcs11Handler::enumCertificates (TokenHandler* token) {
	...
	if (login(session, token)) {
		...
		MessageBox (m_hParentWindow, "PIN incorrecte", "Targeta criptogràfica", MB_OK|MB_ICONINFORMATION);
	}
...
std::vector<TokenHandler*>& Pkcs11Handler::enumTokens () {
	...
	if (WAIT_OBJECT_0 == WaitForSingleObject(m_hMutex, INFINITE))  // no time-out interval
	{
		...
		rv = m_p11->C_GetSlotList(true, p11_slots, &p11_num_slots);
		...
		for (unsigned int n = 0; n < p11_num_slots; n++) {
			...
			if (rv != CKR_OK) {
				m_log.info ("Slot %d: (ERROR)\n", n);
				continue;
			}
			if (!(info.flags & CKF_TOKEN_PRESENT)) {
				m_log.info ("Slot %d: (empty)\n", n);
				continue;
			}

			m_log.info ("Slot %d: %s", n, p11_utf8_to_local(info.slotDescription,
...
void Pkcs11Handler::setNotificationHandler(NotificationHandler *provider)
{
	m_provider = provider;
}
...
bool Pkcs11Handler::getSavedCredentials(CertificateHandler *cert, std::wstring &szUser, std::wstring &szPassword) {
	bool found = false;
	...
	if (WAIT_OBJECT_0 == WaitForSingleObject(m_hMutex, INFINITE))  // no time-out interval
	{
		...
		// Ignorar fallo de autenticación
		if (!login(session, cert->getTokenHandler())) {
			ReleaseMutex(m_hMutex);
			return FALSE;
		}
...
bool Pkcs11Handler::saveCredentials(CertificateHandler *cert, std::wstring &szUser, std::wstring &szPassword) {
	if (WAIT_OBJECT_0 == WaitForSingleObject(m_hMutex, INFINITE))  // no time-out interval
	{
		CK_SESSION_HANDLE session;
		CK_RV rv = m_p11->C_OpenSession(cert->getTokenHandler()->getSlot_num(),
		...
		int len1 = szUser.size()+ 1;
		int len2 = szPassword.size() + 1;
		int len = sizeof(wchar_t) * (len1 + len2);
		wchar_t *wchContent = (wchar_t*) malloc(len);
		wcscpy(wchContent, szUser.c_str());
		wcscpy(&wchContent[len1], szPassword.c_str());
