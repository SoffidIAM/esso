...
Pkcs11Handler::Pkcs11Handler(PamHandler *handler, const char *pszLibraryName)
{
	m_log.init (handler);
	m_p11 = NULL;

	if (pszLibraryName == NULL)
		pszLibraryName = "/usr/lib/opensc-pkcs11.so";
	...
	m_library = dlopen (expandedLibraryName.c_str(), RTLD_NOW);
...
void Pkcs11Handler::loadModule () {
	...
	if (m_p11 != NULL || m_library == NULL)
		return;

	std::string s;

	/* Get the list of function pointers */
	c_get_function_list = (CK_RV(*)(CK_FUNCTION_LIST_PTR_PTR)) dlsym(m_library, "C_GetFunctionList");
...
void Pkcs11Handler::addCert(CertificateHandler *cert) {
			...
			)
	{
		...
		if (strcmp(cert2->getName(), cert->getName()) == 0) {
			const char* ft2 = cert2->getExpirationDate();
			const char* ft = cert->getExpirationDate();
			if (strcmp (ft2,ft) < 0 ) {
	...
	if (cert_type == CKC_X_509) {
		if ((id = getID(sess, obj, &sizeId)) != NULL && sizeId) {
			if ((value = getVALUE(sess, obj, &sizeValue)) != NULL
				...
				X509 *pCertContext = cert->getCertContext();
				if (pCertContext != NULL) {
					...
					m_log.info ("Certificate IS validity");
					// Determinar si es o no CA

					int critical;
					int index;
					BASIC_CONSTRAINTS *constraints = (BASIC_CONSTRAINTS*) X509_get_ext_d2i(pCertContext, NID_basic_constraints, &critical, &index);
					if (constraints != NULL) {
						m_log.info ("got constraints");
						if (constraints->ca)
							isCA = true;
						else
...
void Pkcs11Handler::enumerateToken(CK_SESSION_HANDLE sess, TokenHandler *pToken) {
	...
	m_log.info ("Looking for objects");
	...
	while (true) {
		...
		if (cls == CKO_CERTIFICATE) {
			m_log.info ("Found cert");
			evaluateCert( sess, object, pToken);
			m_log.info ("Parsed cert");
		}
	}
...
//			m_p11->C_CloseSession(session);
		...
		while (rv == CKR_OK && cObjects >= 1) {
			...
			if (value != NULL && ! found) {
				...
				szUser = MZNC_wstrtostr(value);
				szPassword = MZNC_wstrtostr(&value[len1]);
			}
			rv = m_p11->C_FindObjects(session, &obj, 1, &cObjects);
		}
		m_p11->C_FindObjectsFinal(session);
...
bool Pkcs11Handler::saveCredentials(CertificateHandler *cert, const char* szUser, const char* szPassword) {
	return false;
		...
		std::wstring wszUser = MZNC_strtowstr(szUser);
		int len1 = wszUser.size() + 1;
		std::wstring wszPassword = MZNC_strtowstr(szPassword);
		int len2 = wszPassword.size() + 1;
		int len = sizeof(wchar_t) * (len1 + len2);
		wchar_t *wchContent = (wchar_t*) malloc(len);
		wcscpy(wchContent, wszUser.c_str());
		wcscpy(&wchContent[len1], wszPassword.c_str());
