...
bool NossoriHandler::performRequest() {
    ...
    if (response != NULL)
    {
    	...
    	if (status == "OK")
    	{
    		...
    		msg1 += L":" ;
    		msg1 += msg2;
    		MessageBoxW (NULL, msg2.c_str(), L"Soffid SSO", MB_ICONEXCLAMATION|MB_OK);
        	delete response;
    		return false;
    	}
    } else {
		MessageBox (NULL, Utils::LoadResourcesString(5).c_str(), "Soffid SSO", MB_ICONEXCLAMATION|MB_OK);
		return false;
    }
}
...
bool NossoriHandler::responseChallenge() {
    ...
    if (response != NULL)
    {
    	...
    	if (status == "OK")
    	{
    		...
    		MessageBox (NULL, Utils::LoadResourcesString(6).c_str(), "Soffid SSO", MB_ICONEXCLAMATION|MB_OK);
        	delete response;
        	return false;
    	} else {
    ...
    if (response != NULL)
    {
    	...
    	if (status == "OK")
    	{
    	...
    	} else if (status == "BADPASSWORD")
       	{
    		...
    		msg1 += L":" ;
    		msg1 += msg2;
    		MessageBoxW (NULL,msg1.c_str(),  L"Soffid SSO", MB_ICONEXCLAMATION|MB_OK);
        	delete response;
    		return false;
    	}
    } else {
...
bool NossoriHandler::perform()
{
    ...
    if (!SeyconCommon::readProperty("addon.retrieve-password.right_number", v) ||
    		v.empty())
    {
    	MessageBox (NULL, Utils::LoadResourcesString(8).c_str(), "Soffid SSO", MB_ICONEXCLAMATION|MB_OK);
    	return false;
    }
	...
	MessageBox (NULL, Utils::LoadResourcesString(7).c_str(), "Soffid SSO", MB_ICONEXCLAMATION|MB_OK);
	return true;
}
